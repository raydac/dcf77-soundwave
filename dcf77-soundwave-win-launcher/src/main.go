...
//go:generate goversioninfo -64 -gofile=versioninfo.go -o=resource_windows.syso

const (
	ERROR_ALREADY_EXISTS = 183
	JDK_PATH             = "\\jre\\bin\\javaw.exe"
	JAR_FILE             = "\\dcf77soundwave.jar"
	MUTEX                = "dcf77-soundwave"
)
...
func main() {
	...
	if err == nil {
		...
		cmd := exec.Command(base_folder+JDK_PATH, "-Xmx1G", "-jar", base_folder+JAR_FILE)
